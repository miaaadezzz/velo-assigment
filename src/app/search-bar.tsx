...
export default function SearchBar({ stations }: { stations: Station[] }) {
  ...
  const [stationsWithDistance, setStationsWithDistance] =
    useState<Station[]>(stations);
  const [sortOption, setSortOption] = useState<"distance" | "bikes">(
    "distance",
  );
    ...
    <>
      <div className="w-full max-w-md flex flex-row justify-between items-center mb-2">
        ...
        <select
          value={sortOption}
          onChange={(e) =>
            setSortOption(e.target.value as "distance" | "bikes")
          }
